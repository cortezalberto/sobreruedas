...
- **Estado**: ✅ **completo — 52/52 tareas** (23-ago-2026). Suite **943 → 995 tests**, 0 fallas. Cobertura **99.36 % líneas / 98.20 % ramas** (sube desde 99.35/98.17). `ruff`, `black` y `mypy .` limpios sobre 146 archivos — verificados, no solo reportados.
  ...
  - ~~⛔ **Falta**: `vehicle.updated` y la migración `vehicle_status_history`~~ ✅ **CERRADO el 24-ago-2026, completo.** La tabla existe y `HistorialDeEstado` pasó a `changed_by`/`changed_at` sin `notes` (gana N1, `spec-tecnica` §3.4). El append-only por privilegio pedido por la KB llegó **completo**: `DELETE` revocado por `019`, `UPDATE` revocado por `020` (ver el bullet ✅ de arriba, ya cerrado). El `PATCH` que expone `vehicle.updated` por HTTP es C-15.
