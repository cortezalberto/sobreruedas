...
- **Estado**: `[~]` **en curso — 60 de 83 tareas** (actualizado 14-ago-2026, tarea 10.5)
  ...
  - 🔲 **Bloque 9 (`T-008`, 22 tareas) — desbloqueado el 17-ago-2026.** La decisión de proveedor cloud que lo trababa **quedó sin efecto**: [`ADR-023`](docs/adr/ADR-023-despliegue-sobre-vps-con-docker-compose.md) supersede a `ADR-015` y fija **VPS único en Hostinger con Docker Compose**. No hay Terraform, ni Kubernetes, ni ArgoCD, ni "AWS o GCP" que decidir. El bloque se reescribió entero: 16 tareas → **22**, porque backup, parcheo y certificados dejan de ser casilla del proveedor y pasan a ser trabajo propio. ⚠️ Arrastra un **conflicto abierto con el plan de SRE** (nodo único vs. *"DR en región alternativa"* y 99.9 % de Enterprise), escalado a **Dirección + SRE** — ver `ADR-023` §Conflicto declarado con N3.
