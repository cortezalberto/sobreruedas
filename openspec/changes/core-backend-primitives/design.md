...
- No se define la **matriz** de permisos por recurso (riesgo `R-2`): se define el mecanismo que la aplicará. Qué puede cada rol sobre cada recurso no existe en ningún documento vinculante y no se inventa acá.
...
| Riesgo | Mitigación |
|---|---|
| **`R-2` — no existe matriz RBAC canónica.** `require_permission()` se construye sin saber qué permisos existen. | Se entrega el mecanismo con el catálogo de **roles** (que sí está decidido) y una definición de permisos vacía pero consultable. Los permisos finos se pueblan cuando exista la matriz. Se deja registrado que ningún endpoint puede depender de `require_permission` hasta entonces. |
...
> **Lo que NO es una pregunta abierta**: la matriz de permisos (`R-2`) y el catálogo de roles (`E-001`). Los dos cambiarían las tareas, así que están tratados arriba como bloqueante y riesgo, no diferidos acá.
