...
Decisiones tomadas por falta de fuente unívoca. Están acá para que se revisen, no para darlas por firmes.

1. **`docs/` alberga el corpus fuente y la estructura del §4.1 a la vez.** El §4.1 pide `docs/adr/`, `docs/runbooks/` y `docs/openapi.yaml`, pero asumía un `docs/` vacío — acá ya viven los 11 documentos convertidos. **Supuesto**: los 11 `.md` se mueven a `docs/sdd/` y el resto queda como manda el §4.1. Así se cumple la norma vinculante sin desvío y sin ADR justificatorio. Requiere actualizar los enlaces de `knowledge-base/`, `CLAUDE.md` y `AGENTS.md`.
2. **`decisions/ADR-000` se muda a `docs/adr/`.** El §4.1 describe `docs/adr/` como *"ADRs nuevos posteriores al SDD"*, que es exactamente lo que `ADR-000` es. La ubicación original se eligió antes de leer el §4.1.
